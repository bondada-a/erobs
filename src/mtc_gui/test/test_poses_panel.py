...
def test_save_pose_persists_refreshes_and_emits(registry):
    f = registry
    panel = PosesPanel()
...
def test_delete_pose_persists_and_keeps_siblings(registry):
    f = registry
    panel = PosesPanel()
    panel.load_from_file(f)
...
def test_writer_matches_mcp_flow_format(registry):
    f = registry
    panel = PosesPanel()
    panel.load_from_file(f)
