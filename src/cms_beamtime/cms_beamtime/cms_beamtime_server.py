...
class cmsBeamtimeServer(Node):
    ...
    def execute_cleanup(self, mode: str | None = None):
        ...
        self.get_logger().info(f"[Cleanup] Mode={mode} handling state {state_name}")
        ...
        for pname in reversed(executed):
            if pname in pose_map:
                self.inner_sm.move_robot(pose_map[pname], self.path_constraints)
    ...
    def execute_cb(self, goal_handle):
        ...
        for step in raw_sequence:
            ...
            if stype == "move":
                ...
                sequence_steps.append(
                    {"type": "end_effector", "device": step.get("device"), "action": step.get("action")}
                )
            else:
        ...
        success = self.inner_sm.move_robot(home, self.path_constraints)
        if not success:
        ...
        while idx < total:
            ...
            try:
                if stype == "move_single":
                    ...
                    self.get_logger().info(f"[Action Server]  Step {idx+1}/{total} → gripper {action}")
                    if device == "gripper":
                        if action == "open":
