...
try:
    ...
    try:
        ...
        print("Warning: Could not import MTCExecution action. Creating mock class.")
        # Create a mock action class
        class MTCExecution:
            class Goal:
                def __init__(self):
                    self.task_script_json = ""
                    self.robot_ip = ""
                    self.start_gripper = ""
            class Feedback:
                def __init__(self):
    ...
    class ActionClient:
        def __init__(self, node, action_type, action_name):
            ...
            print(f"Mock ActionClient created for {action_name}")
        def wait_for_action_server(self, timeout_sec): 
            print(f"Mock: Waiting for action server {self.action_name}")
            return True
        def send_goal_async(self, goal): 
            print(f"Mock: Sending goal to {self.action_name}")
            class MockFuture:
                def __init__(self):
                    self.done = lambda: True
                    self.result = lambda: type('MockHandle', (), {'accepted': True})()
            return MockFuture()
        def get_result_async(self, goal_handle):
            print(f"Mock: Getting result from {self.action_name}")
            class MockFuture:
                def __init__(self):
                    self.done = lambda: True
                    self.result = lambda: type('MockResult', (), {'result': type('MockResultData', (), {'success': True, 'completed_steps': 1, 'total_steps': 1})()})()
            return MockFuture()
        def cancel_goal_async(self, goal_handle): 
            print(f"Mock: Cancelling goal in {self.action_name}")
            pass
...
class MTCGUIClient:
    def get_logger(self):
        """Get logger - handles both ROS2 and demo mode"""
    ...
    def get_parameter(self, name, default):
        """Get parameter - handles both ROS2 and demo mode"""
        if hasattr(self, 'logger'):
            return default
        if hasattr(self, '_node'):
            return self._node.get_parameter(name, default)
        return default
    
    def declare_parameter(self, name, default):
        """Declare parameter - handles both ROS2 and demo mode"""
        if hasattr(self, 'logger'):
            return
        if hasattr(self, '_node'):
            return self._node.declare_parameter(name, default)
        return None
    
    def destroy_node(self):
        """Destroy node - handles both ROS2 and demo mode"""
        if hasattr(self, 'logger'):
            return
        if hasattr(self, '_node'):
    ...
    def __init__(self):
        self.ros2_available = rclpy_available
        
        if self.ros2_available:
            # Create a proper ROS2 node
            self._node = Node('mtc_gui_client')
            self.name = self._node.name
            self.logger = self._node.get_logger()
        else:
            # If ROS2 is not available, create a mock node
        ...
        # Action client
        self._action_client = ActionClient(self, MTCExecution, 'mtc_execution')
        ...
        # Start ROS2 spin in background only if available
        if self.ros2_available:
    ...
    def setup_gui(self):
        ...
        self.root = tk.Tk()
        self.root.title("MTC Action Client GUI")
    ...
    def create_robot_config_frame(self):
        ...
        ttk.Button(config_frame, text="Test Connection", 
                  command=self.test_connection).grid(row=0, column=4, padx=(20, 0))
    ...
    def execute_task(self):
        """Execute the current task configuration"""
    ...
    def _execute_task_thread(self):
        ...
        try:
            if not self.ros2_available:
                ...
                self.log_message("⚠ Running in demo mode - simulating task execution")
                self.log_message(f"Task configuration: {len(self.current_config.get('sequence', []))} steps")
                
                # Simulate progress
                for i, step in enumerate(self.current_config.get('sequence', [])):
                    if self.stop_execution:
                        self.log_message("Task execution cancelled by user")
                        break
                    
                    self.log_message(f"Executing step {i+1}: {step.get('action', 'unknown')}")
                    time.sleep(1)  # Simulate execution time
                
                if not self.stop_execution:
                    self.log_message("✓ Demo task completed successfully!")
                return
            
            # Wait for action server
            if not self._action_client.wait_for_action_server(timeout_sec=10.0):
                self.log_message("ERROR: Action server not available")
                return
            
            # Create goal
            goal_msg = MTCExecution.Goal()
            goal_msg.task_script_json = json.dumps(self.current_config)
            goal_msg.robot_ip = self.robot_ip_var.get()
            goal_msg.start_gripper = self.start_gripper_var.get()
            
            # Send goal
            self.log_message("Sending task execution goal...")
            goal_handle_future = self._action_client.send_goal_async(goal_msg)
            
            # Wait for goal acceptance
            try:
                rclpy.spin_until_future_complete(self, goal_handle_future, timeout_sec=10.0)
            except Exception:
            ...
            if not goal_handle.accepted:
                self.log_message("ERROR: Goal was rejected")
                return
            
            self.log_message("Goal accepted, task execution started")
            
            # Get result
            result_future = self._action_client.get_result_async(goal_handle)
            
            # Monitor progress
            while not result_future.done():
                if self.stop_execution:
                    self._action_client.cancel_goal_async(goal_handle)
                    self.log_message("Task execution cancelled by user")
                    break
            ...
            if not self.stop_execution:
                ...
                if result.result.success:
                    self.log_message(f"Task completed successfully! ({result.result.completed_steps}/{result.result.total_steps} steps)")
                else:
                    self.log_message(f"Task failed: {result.result.error_message}")
    ...
    def test_connection(self):
        """Test connection to action server"""
        def test_thread():
            try:
                if not self.ros2_available:
                    self.log_message("⚠ Running in demo mode - no actual connection")
                    return
                
                if self._action_client.wait_for_action_server(timeout_sec=5.0):
                    self.log_message("✓ Action server connection successful")
                else:
                    self.log_message("✗ Action server connection failed")
            except Exception as e:
    ...
    def show_about(self):
        ...
        messagebox.showinfo("About", 
                          ...
                          "Allows you to create, edit, and execute robot task sequences without JSON files.\n\n"
                          "Version: 1.0")
