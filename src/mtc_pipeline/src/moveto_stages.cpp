...
bool MoveToStages::run(const nlohmann::json& step, const nlohmann::json& poses) {
  ...
  bool success;
  {
    ...
    if (target_type == "named_state") {
      ...
      task.add(createNamedStateMoveStage("move_to_" + named_state, named_state, planner, arm_group_name));
      ...
      if (planning_type == "cartesian") {
        ...
        // Joint planning
        task.add(createJointMoveStage(label, joint_angles_deg, planner, arm_group_name));
      }
      ...
      task.add(createRelativeMoveStage(label, direction, distance, planner, arm_group_name));
    } else {
