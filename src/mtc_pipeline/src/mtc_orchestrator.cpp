...
void declare_if_needed(rclcpp::Node::SharedPtr node, const std::string& name)
{
    if (!node->has_parameter(name))
        node->declare_parameter(name, std::string{""});          // declare as empty string
}

/* --- sync robot + planning params (now includes planning_plugin!) --- */
                                   ...
                                   rclcpp::Node::SharedPtr node)
{
    ...
    /* pull description + planning parameters from move_group */
    ...
    /* declare locally before setting */
    ...
    RCLCPP_INFO(node->get_logger(),
                "Synchronized robot + planning params (planner = %s) from [%s]",
                !plugin.empty() ? plugin.c_str()
                                : ompl_plugin.c_str(),
                source_node.c_str());
    return true;
}
...
{   
    ...
    if (!fut.get()->success) {
        RCLCPP_ERROR(node->get_logger(), "Dashboard 'play' responded with failure");
        return false;
    }
...
public:
    pid_t launch(const std::string& cmd)
    {
        pid_t pid = fork();
        if (pid == 0) {
            execl("/usr/bin/setsid", "setsid", "bash", "-c",
                       ...
                       int max_tries = 20, int interval_sec = 1)
    {
        for (int i = 0; i < max_tries; ++i) {
            ...
            while (fgets(buf, 128, pipe)) {
                if (std::string(buf).find(node_name) != std::string::npos) {
                    found = true; break;
                }
            }
            pclose(pipe);
    ...
    std::string get_current_gripper() const { return current_gripper_; }
};
...
int main(int argc, char** argv)
{
    ...
    auto node = rclcpp::Node::make_shared("mtc_orchestrator", opts);
    ...
    if (!ifs) {
        RCLCPP_ERROR(node->get_logger(), "Unable to open config file: %s", cfg_file.c_str());
        return 1;
    }
    nlohmann::json cfg; ifs >> cfg;
    if (!cfg.contains("poses") || !cfg.contains("sequence")) {
        RCLCPP_ERROR(node->get_logger(), "JSON must contain 'poses' and 'sequence'");
        return 1;
    }
    const auto& poses     = cfg["poses"];
    const auto& sequence  = cfg["sequence"];

    /* ----- launch first MoveIt stack (no gripper) ----- */
    orch.kill_all_and_wait();
    orch.launch(launch_cmd_for_gripper("none"));
    ...
    for (const auto& step : sequence) {
        ...
        if (action == "tool_exchange")
        {
            ...
            /* attach new tool stack */
            orch.kill_all_and_wait();
            orch.launch(launch_cmd_for_gripper(new_gripper));
            orch.wait_for_node("move_group");
            if (!play_dashboard_client(node) ||
                !update_robot_description_from("move_group", node))
                goto failed;
        ...
        else if (action == "pick_and_place")
        {
            ...
            if (need != orch.get_current_gripper()) {
                ...
                orch.launch(launch_cmd_for_gripper(need));
                orch.wait_for_node("move_group");
                if (!play_dashboard_client(node) ||
                    !update_robot_description_from("move_group", node))
                    goto failed;
        ...
        failed:
            ...
            std::this_thread::sleep_for(std::chrono::seconds(60));  // <--- Pause here!
            orch.kill_all_and_wait();
            rclcpp::shutdown();
            return 1;
        }
