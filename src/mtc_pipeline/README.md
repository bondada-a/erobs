...
Task stages do not explicitly specify controllers, allowing MoveIt to auto-select from `moveit_controllers.yaml`. This keeps controller configuration centralized and makes it easier to switch controllers without modifying stage implementations. The warning "does not have any controllers specified for trajectory execution" is cosmetic - MoveIt successfully selects and executes with the appropriate controller.
...
`[WARN] [moveit.ompl_planning]: It looks like the planning volume was not specified` can be eliminated by adding `default_planner_request_adapters/FixWorkspaceBounds` to the request_adapters list in `ompl_planning.yaml`. This adapter adds default 10m³ workspace bounds when not specified, making planning slightly more efficient. Currently not critical as planning succeeds with unbounded search.
