...
def test_cartesian_stages_require_complete_paths(operation, current_gripper):
    ...
    exchange.create_task_template = Mock(return_value=SimpleNamespace(add=Mock()))
    ...
    sequence = [
        ...
        {"direction": "backward", "distance": 0.2},
    ]
    config = {
        "dock_spacing_m": 0.1524,
        "reference_dock": 3,
        "load_sequence": sequence,
        "dock_sequence": sequence,
    }
    ...
    with (
        patch.object(
            tool_exchange_stages, "_load_tool_exchange_config", return_value=config
        ),
        patch.object(tool_exchange_stages.stages, "MoveTo", return_value=move_to),
    ):
        assert exchange.run(goal) is None

    planners = [
        call.args[-1] for call in exchange.create_relative_move_stage.call_args_list
    ]
