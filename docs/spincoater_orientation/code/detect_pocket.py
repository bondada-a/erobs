#!/usr/bin/env python3
"""Empty-pocket detector for the red-filled spincoater chuck.

Pipeline: find red field (HSV dual-range) -> restrict to field ->
isolate bright bare-metal square (high V, low S) -> minAreaRect ->
return (center_px, angle_mod90, aspect, solidity, area).

Usage: detect_pocket.py <image> [cx cy half]
Auto-locates the chuck via the red field if cx/cy not given.
"""
import sys, cv2, numpy as np

def red_mask(hsv):
    m1 = cv2.inRange(hsv, (0, 60, 40), (14, 255, 255))
    m2 = cv2.inRange(hsv, (166, 60, 40), (179, 255, 255))
    return cv2.bitwise_or(m1, m2)

def locate_chuck(img):
    """Find chuck center as centroid of the largest red blob in full frame."""
    hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
    red = red_mask(hsv)
    red = cv2.morphologyEx(red, cv2.MORPH_CLOSE, np.ones((25, 25), np.uint8))
    red = cv2.morphologyEx(red, cv2.MORPH_OPEN, np.ones((9, 9), np.uint8))
    cnts, _ = cv2.findContours(red, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    if not cnts:
        return None
    c = max(cnts, key=cv2.contourArea)
    M = cv2.moments(c)
    if M["m00"] == 0:
        return None
    cx, cy = int(M["m10"] / M["m00"]), int(M["m01"] / M["m00"])
    # half = radius-ish from area
    r = int(np.sqrt(cv2.contourArea(c) / np.pi))
    return cx, cy, max(120, int(r * 1.4)), cv2.contourArea(c)

def detect(img, cx, cy, half):
    x0, y0 = max(0, cx - half), max(0, cy - half)
    roi = img[y0:y0 + 2 * half, x0:x0 + 2 * half]
    hsv = cv2.cvtColor(roi, cv2.COLOR_BGR2HSV)
    red = red_mask(hsv)
    redc = cv2.morphologyEx(red, cv2.MORPH_CLOSE, np.ones((21, 21), np.uint8))
    cnts, _ = cv2.findContours(redc, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    if not cnts:
        return None
    field = max(cnts, key=cv2.contourArea)
    fmask = np.zeros_like(red)
    cv2.drawContours(fmask, [field], -1, 255, -1)
    bright = cv2.inRange(hsv, (0, 0, 150), (179, 90, 255))
    bright = cv2.bitwise_and(bright, fmask)
    bright = cv2.morphologyEx(bright, cv2.MORPH_OPEN, np.ones((5, 5), np.uint8))
    bright = cv2.morphologyEx(bright, cv2.MORPH_CLOSE, np.ones((9, 9), np.uint8))
    cnts, _ = cv2.findContours(bright, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    for c in sorted(cnts, key=cv2.contourArea, reverse=True):
        a = cv2.contourArea(c)
        if a < 1200:
            continue
        (rx, ry), (rw, rh), ang = cv2.minAreaRect(c)
        asp = max(rw, rh) / (min(rw, rh) + 1e-6)
        sol = a / (cv2.contourArea(cv2.convexHull(c)) + 1e-6)
        if asp < 1.25 and sol > 0.85:
            return dict(cx=rx + x0, cy=ry + y0, angle=ang % 90,
                        w=rw, h=rh, asp=asp, sol=sol, area=a)
    return None

if __name__ == "__main__":
    img = cv2.imread(sys.argv[1])
    if len(sys.argv) >= 5:
        cx, cy, half = int(sys.argv[2]), int(sys.argv[3]), int(sys.argv[4])
        loc_area = None
    else:
        loc = locate_chuck(img)
        if loc is None:
            print("FAIL: no red field found"); sys.exit(1)
        cx, cy, half, loc_area = loc
    r = detect(img, cx, cy, half)
    if r is None:
        print(f"FAIL: no pocket (chuck@{cx},{cy} half={half})")
    else:
        print(f"OK center=({r['cx']:.0f},{r['cy']:.0f}) angle_mod90={r['angle']:.1f} "
              f"wh=({r['w']:.0f},{r['h']:.0f}) asp={r['asp']:.2f} sol={r['sol']:.2f} "
              f"area={r['area']:.0f}  [chuck@{cx},{cy} half={half}]")
