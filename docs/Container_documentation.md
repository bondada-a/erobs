...
**Note**: Joint poses are in **degrees**, converted to radians internally. The `"poses"` dict is optional — the orchestrator auto-resolves named poses (e.g. `"home"`, `"pickup_approach"`) from the beamline's pose registry (`poses_file` in the active beamline YAML, e.g. `cms_beamline.yaml`) when they are not supplied inline.
