...
\noindent\textbf{bsui Container Launch Command:}
...
\subsection{Ubuntu Workstation Implementation}
\begin{enumerate}
    \item Native installation - no Docker containers, all components run directly on the workstation
    % \item Full MTC orchestrator with multiple action servers (move\_to, pick\_place, vision, etc.)
    \item Direct camera integration via Camera Node (Zivid 10GbE, ZED USB)
    \item GPU available for camera point cloud processing
\end{enumerate}
...
%% LEFT SIDE - Bluesky components %%
\node (runengine) [componentbox] {
    \textbf{Bluesky RunEngine}
};
...
%% RIGHT SIDE - ROS2 components %%
\node (orchestrator) [componentbox, right=3cm of runengine] {
    \textbf{MTCOrchestrator}\\[0.05cm]
    \textbf{Server}
};
...
\node (camera_node) [
    componentbox,
    right=2.5cm of action_servers,
    fill=cameracolor!70,
    minimum height=1.8cm
] {
    \textbf{Camera Node}\\[0.1cm]
    {\small (Zivid / ZED)}
};

%% CAMERA HARDWARE (right of workstation) %%
\node (camera) [
    right=2cm of camera_node,
    rectangle,
    rounded corners=6pt,
    minimum width=4cm,
    minimum height=1.5cm,
...
%% WORKSTATION LAYER - Draw workstation box %%
\begin{pgfonlayer}{vmlayer}
    \node (workstation_box) [
        rectangle,
        rounded corners=12pt,
        ...
        draw=black,
        line width=2pt,
        fit=(runengine)(ophyd)(orchestrator)(action_servers)(moveit)(camera_node),
        inner sep=1cm
    ] {};
\end{pgfonlayer}

%% Workstation Label %%
\node [anchor=north west, font=\Large\bfseries, xshift=0.3cm, yshift=-0.3cm] at (workstation_box.north west) {Ubuntu Workstation};
...
%% Camera communication arrows - fan out pattern %%
\draw [arrow] (camera_node.north) |- (orchestrator.east);
\draw [arrow] (camera_node.west) -- (action_servers.east);
\draw [arrow] (camera_node.south) |- (moveit.east);

%% Camera hardware connection %%
\draw [arrow] (camera.west) --
    node[pos=0.5, above, font=\small\bfseries] {10GbE / USB}
    (camera_node.east);

\end{tikzpicture}
\caption{EROBS deployment architecture on Ubuntu Workstation.}
\label{fig:workstation-architecture}
\end{figure}

\subsection{Laptop Implementation}
\begin{enumerate}
    \item Native installation - no Docker containers, all components run directly on the laptop
    % \item Full MTC orchestrator with multiple action servers (move\_to, pick\_place, vision, etc.)
    \item Direct camera integration via Camera Node (Zivid 10GbE, ZED USB)
    \item GPU available for camera point cloud processing
\end{enumerate}
...
%% LEFT SIDE - GUI / Action Client %%
\node (gui_client) [componentbox, minimum height=1.8cm] {
    \textbf{GUI / Action Client}\\[0.1cm]
    {\small (ROS2 Action Client)}
};

%% RIGHT SIDE - ROS2 components %%
\node (orchestrator) [componentbox, right=3cm of gui_client] {
    \textbf{MTCOrchestrator}\\[0.05cm]
    \textbf{Server}
};
...
%% CAMERA HARDWARE (right of laptop) %%
\node (camera) [
    right=2cm of camera_node,
    rectangle,
    rounded corners=6pt,
...
\begin{pgfonlayer}{vmlayer}
    \node (laptop_box) [
        ...
        draw=black,
        line width=2pt,
        fit=(gui_client)(orchestrator)(action_servers)(moveit)(camera_node),
        inner sep=1cm
    ] {};
\end{pgfonlayer}

%% Laptop Label %%
\node [anchor=north west, font=\Large\bfseries, xshift=0.3cm, yshift=-0.3cm] at (laptop_box.north west) {Laptop};

%% ARROWS %%
\draw [arrow] (gui_client.east) --
    node[pos=0.5, above, font=\small\bfseries] {JSON task}
    (orchestrator.west);
...
\end{tikzpicture}
\caption{EROBS deployment architecture on Laptop.}
\label{fig:laptop-architecture}
\end{figure}
